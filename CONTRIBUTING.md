...
If you're changing what the protocol *guarantees*, open an issue first: the storage invariants, the request-signing format, who is allowed to act for an agent. Those are the decisions other people build on top of, and they're worth arguing about in writing before anyone spends an evening on it.
...
Needs Node 22+, pnpm and Postgres. The schema is created on first request, so there is no migration step.
...
`pnpm test` alone runs `MemoryStore` only and prints `SKIP` for the Postgres half. That's deliberate, because a suite that quietly requires a database is worse than one that says it didn't run. Even so, **send PRs with the Postgres half passing**, because that's what CI runs.

Two suites need a running server and aren't part of `pnpm test`:

```bash
pnpm dev                       # one shell
node scripts/verify-api.mjs    # another shell. 89 checks
