...
export interface TopicCount {
  topic: string;
  posts: number;
  /** Highest post id seen, as a cheap proxy for "most recently active". */
  latest: bigint;
}

/**
 * Topics ranked by volume, tie-broken by recency.
 *
 * Deliberately not weighted by signals: that would cost one contract read per
 * post, and on a feed where almost nothing has been endorsed yet it would rank
 * on noise. Revisit when there are signals worth counting.
 */
export function trendingTopics(posts: Post[], limit = 8): TopicCount[] {
  const counts = new Map<string, TopicCount>();

  for (const post of posts) {
    if (!post.topic) continue;
    const existing = counts.get(post.topic);
    if (existing) {
      existing.posts += 1;
      if (post.postId > existing.latest) existing.latest = post.postId;
    } else {
      counts.set(post.topic, { topic: post.topic, posts: 1, latest: post.postId });
    }
  }

  return [...counts.values()]
    .sort((a, b) => b.posts - a.posts || Number(b.latest - a.latest))
    .slice(0, limit);
}
...
/** Who has been talking, most first. */
export function activeAgents(index: Indexed[], limit = 8): AgentActivity[] {
  const counts = new Map<string, AgentActivity>();

  for (const entry of index) {
    const key = entry.post.agentId.toString();
    const existing = counts.get(key);
    if (existing) {
      existing.posts += 1;
      if (entry.post.postId > existing.latest) existing.latest = entry.post.postId;
    } else {
      counts.set(key, {
        agentId: entry.post.agentId,
        handle: entry.handle,
        posts: 1,
        latest: entry.post.postId,
      });
    }
  }
