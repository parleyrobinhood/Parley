...
check("the two rates are 1/5 and 1/31", [RATES.founding, RATES.ongoing], [5, 31]);

/* The ongoing rate, for an agent that arrived after the snapshot. */
check("no snapshot means everything at 1/31", row({ score: 310 }).target, "10000000");
...
check(
  "  and 1/31 on everything since",
  row({ score: 410, snapshotScore: 100 }).target,
  "30000000",
);
check(
  "  which is more than the ongoing rate alone would give",
  BigInt(row({ score: 410, snapshotScore: 100 }).target) >
    BigInt(row({ score: 410, snapshotScore: null }).target),
  true,
);
...
check(
  ...
  [200, 510, 1130].map((score) => row({ score, snapshotScore: 200 }).target),
...
check("owed is target minus what already arrived", row({ score: 310, received: "4000000" }).owed, "6000000");
check("  fully paid owes nothing", row({ score: 310, received: "10000000" }).owed, "0");
check(
  "  and an overpayment never goes negative",
  row({ score: 310, received: "99000000" }).owed,
  "0",
);
...
check(
  ...
  row({ score: 310, received: "9996500" }).owed,
  "0",
);
check(
  "  and so does anything that displays as 0.00",
  formatUsdg(row({ score: 310, received: "9990001" }).owed),
  "0.00",
);
check(
  "a whole cent is still owed",
  row({ score: 310, received: "9990000" }).owed,
  "10000",
);
check(
  "dust is not forgiven, only deferred: it is paid once it passes the floor",
  row({ score: 341, received: "9996500" }).owed,
  "1003500",
);
...
const sheet = allocateAll([
  { agentId: 1, handle: "payable", score: 310, wallet: "0xa", snapshotScore: null, received: "0", sharedWallet: false, verified: false },
  { agentId: 2, handle: "nowallet", score: 310, wallet: null, snapshotScore: null, received: "0", sharedWallet: false, verified: false },
  { agentId: 3, handle: "shared", score: 310, wallet: "0xb", snapshotScore: null, received: "0", sharedWallet: true, verified: false },
]);
